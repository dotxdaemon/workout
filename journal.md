...
- Publication: the first HTTPS push failed because Git had no credential helper configured even though GitHub CLI authentication was valid; `gh auth setup-git` restored the existing keyring-backed helper and the retry pushed `main` successfully.

2026-09-22 (phone-first logging and data integrity)
- Sean requested implementation and verification of a simpler phone-first logging flow, prioritizing data safety. Baseline at `1f4a7ee`: isolated original checkout passed 102 tests, lint, typecheck, and build. No production records were opened or modified; no push or deployment was authorized or performed.
- Root causes: startup could close unfinished sessions and merge seeded identities; current plans followed mutable routines; old sets inherited mutable unit defaults; unfinished/template entries influenced history/progression; completion lacked stable retry identity; correction writes and draft transitions could lose newer edits. Import needed rollback across database/preferences and pending drafts. Imported timezone offsets were sorted as strings.
- Implementation `c4af7fe`: retain active sessions across dates with explicit Finish; snapshot session exercise IDs; retain stable exercise identity on rename; remove destructive bootstrap repair; Dexie v3 snapshots each set's interpreted unit without changing values. Completed-set creation is idempotent and transactional, corrections retain identity/order/date, imports validate first and roll back failures, dates compare actual instants.
- Logging now expands one exercise, labels Weight/Reps, separates completion from next-set drafts, autosaves valid corrections with truthful local-save feedback, and keeps failed/pending input durable. Single Add exercise search/create entry point; secondary removal/notes and separate routine editing. History shows dated actual sets and unit-aware corrections; estimates live in a disclosure. Progression excludes unfinished/warm-up work, compares units correctly, and targets the first lowest-rep set unless all target sets qualify for a load increase.
- Regression work included failing-first coverage for autosave races, reload/navigation drafts, precision, failed correction/cancel storage, quota failure, import rollback, unit migration, routine/history identity, and offset date ordering. Latest complete run: `npm test` 161/161 (17 files), `npm run lint`, `npm run typecheck`, `npm run build`, and `git diff --check` exit 0. Forbidden image-hook search: no matches. Package scripts, dependencies, service worker, and global shell geometry are unchanged.
- Browser evidence: production preview at 360x800, 390x844, 430x932, 1280x900; both themes, Settings/Edit, long exercise names, zero load, several completed sets, repeated taps, correction, finish/reopen/history, sheet internal scroll and Escape focus restoration. At 390px nav remains 779.8125–844 and numeric input text is 20px. No horizontal document overflow. Stopping the preview server, reloading its cached shell, logging, reloading offline, then restoring the server retained all records. Final production console warning/error log empty.
- Before/after screenshots and raw command logs are supplied in the companion verification bundle outside the repository. Physical iOS keyboard, safe-area, touch/gesture, and enlarged OS text checks were unavailable; no device-level claim is made. Existing viewport/sheet regressions pass. Unit changes predating per-set unit storage cannot be reconstructed; ephemeral local input drafts and existing exercise notes are not JSON backup records. Existing dependency audit advisories were not changed in this task.

2026-09-22 (restore one-rep adjustments)
- Sean requested the +1/-1 ability back and authorized committing and pushing. Root cause: the focused-entry redesign replaced the old rep stepper with a direct input. Restored compact minus-one/plus-one buttons beside Reps, reusing existing button styles and the durable correction path. Each tap reads the current draft ref, so batched taps do not overwrite each other; invalid text is preserved and zero is the lower bound. Zero reps still cannot be completed. No schema, service worker, weight controls, or shell changes.
- Implementation `d559fe7`. Two new behavior regressions failed first because the controls were absent; both then passed, covering rapid draft changes without completing work and autosaving corrections to the same completed record. Full suite: 163 tests passed in 17 files. Lint, typecheck, production build, diff check, and forbidden-pattern gate passed.
- Browser: at 390x844, completed-set reps changed 8 to 10 to 9 via rapid clicks, saved as one record, survived reload, then returned to 8. Both buttons measure 44x48px; no horizontal overflow at 360x800 or 390x844, nav bottom remains 844 at 390px. Before/after screenshots are in the companion verification bundle. Real iOS keyboard/touch testing remains unavailable.
2026-09-22 (simplify the logging interface)
- Prior attempt: the focused entry restored +/-1 reps but left a tall header, repeated status rows, boxed controls, and always-visible progression guidance. Sean reported that it looked ugly, did not follow the brief, and asked to remove extraneous things and make it usable.
- Cause: secondary information consumed the same space and emphasis as the actual logging controls. Consolidated Today navigation, removed exercise numbering and empty history captions on collapsed rows, combined entry metadata, gave numeric controls one shared surface, and put notes/removal/progression behind Options. SettingsScreen.tsx also drops redundant captions; shared headings use consistent sizing. Storage, service worker and shell geometry are unchanged.
- Fixed and verified: at 390x844, the comparable completed exercise shrank from about 438px to 321px; all five exercise names fit above navigation. Last performance stays visible; both +/-1 buttons remain 44x48px, inputs 20px. Complete set/Add next set share primary-action emphasis. Notes, removal, and progression remain accessible on request. A review caught and corrected a mismatch between the Use last visible label and accessible name.
- Two new failing-first regressions cover compact day/mode navigation and the Options disclosure. Existing removal/progression tests now exercise opening and closing Options while retaining their data assertions. Final full suite: 165 tests in 17 files, lint, typecheck, build, diff check, and forbidden-pattern gate passed.
- Browser verification: 360x800, 390x844, 430x932, 1280x900, both themes, Settings/Edit/history, add exercise, long wrapped exercise name and zero load. Rapid rep adjustments persisted through reload; next-set drafts stayed unfinished; Finish populated previous performance. Add exercise is fully reachable above the nav; no horizontal overflow; production console warnings/errors empty. Final production screenshot is in the companion bundle. Real iOS keyboard, safe-area and touch testing remain unavailable. Push continues the existing authorized branch workflow.
2026-09-23 (offline cache keeps error responses)
- Sean asked: improve the repo in any way I see fit. Baseline on `main` (`8db5366`): 165 tests, lint, typecheck exit 0.
- Root cause: `public/sw.js` `handleNavigation` wrote every navigation response to `/index.html` and `cacheFirst` wrote every asset response, with no `ok` check (unlike `networkFirst`). One 404/500 page load replaced the offline shell, so the next offline launch opened the error page; a failed icon/manifest fetch was served from cache forever.
- TDD: `src/lib/serviceWorkerCache.test.ts` runs the real `sw.js` (via `?raw`) against an in-memory Cache API. Both tests failed first (offline launch served cached 404; icon stuck on cached 503), then passed after guarding both writes with `networkResponse.ok`. Bumped `CACHE_NAME` to `workout-shell-v6` so the activate handler purges error responses already stored on devices.
- Verification: `npm test` 167/167 (18 files) with no warnings, `npm run lint`, `npm run typecheck`, `npm run build` exit 0; `dist/sw.js` contains the guards. Evidence is unit test only. iOS PWA picks up the new service worker only after the app is closed and reopened; no device check was performed.
2026-09-23 (Train entry visual polish)
- Sean asked: make it look better (after noting the service-worker fix changed nothing visible).
- Problems seen at 390x844: the weight field's placeholder dash read like a minus button; values were small, left-aligned text in a large empty box; -1/+1 were bare text with no button affordance; the exercise being logged was visually identical to collapsed rows.
- Change (CSS only, `src/index.css`, edited the existing Train overrides in place): the active exercise sits on the shared `--control` fill with a jade inset rail; weight/reps are two recessed wells with centered 1.85rem numerals and a faint placeholder; -1/+1 are filled 44px buttons with a distinct disabled state; the primary action grows to fill the action row (50px); completed collapsed exercises dim their name. No markup, text, data, service worker, or shell (.app-shell/.screen-area/.bottom-nav) changes.
- Verification: `npm test` 167/167 with no warnings, lint, typecheck, build exit 0. In-app browser at 390x844 dark + light and 360x800: no horizontal overflow, nav bottom at viewport edge, reps input 60.8px wide at 360px (fits 3 digits), input text 29.6px (no iOS focus zoom). Logged 135 x 8 end to end. No real-device check.
