...
| 2026-03-08 | Bottom nav can be missing on initial load in installed iOS PWA mode | The fixed-height shell did not reserve the standalone top safe area inside the grid, so the bottom nav row could start outside the visible safe region on first paint. | `docs/testing/repro-pwa-startup-nav.js` | `d08c751` | Before: `/tmp/pwa-startup-nav-before.png`, After: `/tmp/pwa-startup-nav-after.png` |
