...
| 2026-03-08 | Bottom nav can be missing on initial load in installed iOS PWA mode | The shell relied only on `100lvh`, so startup environments that do not honor `lvh` leave the nav row below the content instead of pinning it inside the app shell. | `provides a dynamic-viewport fallback before lvh so the startup nav row still renders when lvh is unavailable` | `c4ef9a3` | Before: `/tmp/pwa-startup-nav-before.png`, After: `/tmp/pwa-startup-nav-fallback-after.png` |
